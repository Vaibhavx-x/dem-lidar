...
class CurriculumHMATensorDataset(Dataset):
    def __init__(
        self,
        data_paths,
        mode="train",
        ...
        val_overlap=64,
        val_pad=None,
        photon_focus=0.0,  # Controls the probability of centering crops on photons
        target_photons=0
    ):
        self.mode = mode
        ...
        self.photon_focus = photon_focus
    ...
    def __getitem__(self, idx):
        ...
        # ======================================================================
        # TRAIN MODE: Curriculum Density-Aware Crop
        # ======================================================================
        if self.mode == "train":
            c, h, w = data.shape
            
            # Safely handle inputs smaller than train_crop
            ...
            #------------------------------------------------------------------
            # Curriculum Photon-Centric Crop
            # ------------------------------------------------------------------
            
            mask_layer = data[2]
            photon_coords = np.argwhere(mask_layer > 0)
            
            use_photon_crop = (
                len(photon_coords) > 0 and
                np.random.rand() < self.photon_focus
            )
            ...
            if use_photon_crop:
                ...
                for _ in range(NUM_TRIES):
                    ...
                    cy, cx = photon_coords[
                        np.random.randint(len(photon_coords))
                    ]
            
                    y0 = max(
                        0,
                        min(cy - self.train_crop // 2, max_y)
                    )
            
                    x0 = max(
                        0,
                        min(cx - self.train_crop // 2, max_x)
                    )
            
                    crop_count = int(
                        np.sum(
                            mask_layer[
                                y0:y0+self.train_crop,
                                x0:x0+self.train_crop
                            ]
                        )
                    )
                    ...
                    if crop_count >= self.target_photons:
                        break
        ...
        # ======================================================================
            ...
            data_padded = np.pad(data, ((0, 0), (pad, pad), (pad, pad)), mode="reflect")
            ...
            return {
                ...
                "pad": torch.tensor(pad, dtype=torch.int32),
                "gt_canvas_full": torch.from_numpy(data[3].copy())
            }

def create_curriculum_dataloaders(train_dirs, val_dirs, batch_size=4, num_workers=8, prefetch_factor=4, pin_memory=True):
    train_dataset = CurriculumHMATensorDataset(train_dirs, mode="train", train_crop=128,photon_focus=0.8,target_photons=20)
    val_dataset   = CurriculumHMATensorDataset(val_dirs,   mode="val",   val_crop=256, val_overlap=192)
