...
class CSPNRefine(nn.Module):
    ...
    blending masked (ATL08) pixels with their raw elevation based on a
    learned confidence value. This replaces the hard-reset to avoid
    seeding local artifacts from unreliable anchors.
    """
    ...
    def __init__(self, channels, num_iters=6):
        ...
        # Confidence head: produces c in (0, 1).
        # Inputs: fused dense features (channels) + |dem_coarse - lidar_raw| (1) + normalized patch_mean (1)
        ...
        # Initialize bias to ~4.0 so sigmoid(4) approx 0.98 (starts as a strong anchor)
        nn.init.constant_(self.conf_head[-1].bias, 4.0)

    def forward(self, coarse_dem, dense_feat, lidar_raw, mask, patch_mean=None):
        affinity = F.softmax(self.affinity_head(dense_feat), dim=1)
        H, W = coarse_dem.shape[2], coarse_dem.shape[3]

        # Compute confidence c
        abs_diff = torch.abs(coarse_dem - lidar_raw)
        if patch_mean is None:
            # Fallback if patch_mean isn't provided (for backward compatibility)
            mean_feat = torch.zeros((coarse_dem.shape[0], 1, 1, 1), device=coarse_dem.device)
        else:
            # Expect patch_mean to be [B, 1] or [B, 1, 1, 1]
            if patch_mean.dim() == 2:
                mean_feat = patch_mean.view(-1, 1, 1, 1)
            elif patch_mean.dim() == 1:
                mean_feat = patch_mean.view(-1, 1, 1, 1)
            else:
                mean_feat = patch_mean

        conf_input = torch.cat([dense_feat, abs_diff, mean_feat.expand(-1, -1, H, W)], dim=1)
        c = torch.sigmoid(self.conf_head(conf_input))

        # Soft anchor blend instead of hard reset
        x = mask * (c * lidar_raw + (1 - c) * coarse_dem) + (1 - mask) * coarse_dem
        ...
        for _ in range(self.num_iters):
            ...
            x = mask * (c * lidar_raw + (1 - c) * propagated) + (1 - mask) * propagated
...
class SymGateVDSR(nn.Module):
    ...
    def forward(self, dem_bic, lidar_delta, mask, dist_map, patch_mean=None):
        ...
        dem_refined, affinity, confidence = self.refine(dem_coarse, d, lidar_raw, mask, patch_mean=patch_mean)

        return {
            "dem_pred": dem_refined,      # final output, softly-anchored at ATL08 pixels
...
class SymGateTopoLoss(nn.Module):
    ...
    def __init__(self, alpha=1.0, beta=1.5, gamma=0.5,
                 lambda_pin=1.0, lambda_aux=0.4, lambda_conf=0.1, pin_beta=1.0,
    ...
    def forward(self, outputs, gt_dem, lidar_raw, lidar_delta, mask, dist_map):
        ...
        # 5. confidence regularization - pull mean confidence back toward 1.0
        conf_loss = F.mse_loss(confidence, torch.ones_like(confidence))
...
if __name__ == "__main__":
    ...
    mock_patch_mean = torch.randn(B, 1, device=device)

    optimizer.zero_grad()
    outputs = model(mock_dem_bic, mock_lidar_delta, mock_mask, mock_dist_map, patch_mean=mock_patch_mean)
    loss_dict = criterion(
    ...
    for k in ["total", "base", "slope", "curve", "pin", "aux", "conf"]:
    ...
    print(f"Anchor fidelity (mean abs error at ATL08 pixels, should be small but not 0): "
